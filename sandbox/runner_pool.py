...
class RunnerProcess:
    ...
    async def start(self) -> None:
        ...
        preexec = (lambda: apply_runner_rlimits(self._limits)) if is_rlimit_supported() else None
    ...
    async def execute(self, code: str, *, timeout_seconds: int) -> ExecOutcome:
        ...
        async with self._lock:
            ...
            if limit_kind is None and (stdout_dropped > 0 or stderr_dropped > 0):
        ...
        while True:
            ...
            if not raw:
                ...
                if limit_kind is not None:
                    stderr_suffix = f"runner: killed by sandbox limit ({limit_kind.value})\n"
                else:
