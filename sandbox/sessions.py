...
DEFAULT_GC_INTERVAL_SECONDS = int(os.environ.get("SANDBOX_SESSION_GC_INTERVAL", "30"))
...
class SessionManager:
        ...
        async with self._lock:
            if len(self._sessions) >= self._max_sessions:
                raise SessionLimitExceededError(
                    f"sandbox holds {len(self._sessions)} sessions (max={self._max_sessions})"
                )
            ...
            session = Session(
                ...
                stub_dir=stub_dir,
                runner=runner,
                idle_timeout_seconds=idle_timeout_seconds or DEFAULT_IDLE_TIMEOUT_SECONDS,
                max_lifetime_seconds=max_lifetime_seconds or DEFAULT_MAX_LIFETIME_SECONDS,
            )
    ...
    async def destroy(self, session_id: str) -> None:
        ...
        try:
            ...
            logger.warning("failed to stop runner for session %s", session_id, exc_info=True)
