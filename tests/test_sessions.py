...
def test_two_sessions_have_isolated_python_state(sessions_root: Path) -> None:
    ...
    async def body() -> None:
        ...
        try:
            ...
            outcome_b = await b.runner.execute("print('marker' in dir())", timeout_seconds=5)
            assert outcome_b.return_code == 0
            assert outcome_b.stdout == "False"

            outcome_a = await a.runner.execute("print(session_marker)", timeout_seconds=5)
...
def test_runner_uses_session_workspace_as_cwd(sessions_root: Path) -> None:
    async def body() -> None:
        ...
        try:
            ...
            assert Path(outcome.stdout).resolve() == session.workspace_dir.resolve()
        finally:
            await mgr.shutdown()
